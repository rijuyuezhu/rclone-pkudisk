...
Uploads require a known object size. Small files use AnyShare's signed single-upload flow; larger files use its multipart flow while streaming the source rather than buffering the whole object in memory. When rclone selects its multi-thread copy path (256 MiB and above by default), the backend exposes AnyShare multipart parts through `OpenChunkWriter`, so rclone can upload independent parts concurrently. The usual rclone `--multi-thread-cutoff` and `--multi-thread-streams` controls determine when the multi-thread path is considered. PKU Disk multipart uploads default to four concurrent part uploads; `--pkudisk-upload-concurrency` (or `upload_concurrency` in the remote config) can lower or raise that backend limit. Setting `--multi-thread-streams 1` disables rclone's multi-thread copy path before the backend is consulted.
