...
Uploads require a known object size. Small files use AnyShare's signed single-upload flow; larger files use its multipart flow while streaming the source rather than buffering the whole object in memory. When rclone selects its multi-thread copy path (256 MiB and above by default), the backend exposes AnyShare multipart parts through `OpenChunkWriter`, so rclone can upload independent parts concurrently. The usual rclone `--multi-thread-cutoff` and `--multi-thread-streams` controls determine when the multi-thread path is considered. PKU Disk multipart uploads prefer four concurrent part uploads by default; `--pkudisk-upload-concurrency` (or `upload_concurrency` in the remote config) changes that backend preference. When `--multi-thread-streams` is explicitly larger, rclone may use the larger user-requested concurrency; setting it to `1` disables rclone's multi-thread copy path before the backend is consulted.

Interrupted `OpenChunkWriter` uploads are resumable across rclone process restarts. Minimal state is kept under rclone's cache directory in `pkudisk/multipart`: the AnyShare upload ID, completed part ETags/sizes/SHA-256 digests, source identity/size/mtime, destination parent ID and encoded leaf, and the pre-existing destination ID/revision for updates. Signed object-storage URLs and authorization headers are never persisted. When rclone reaches a previously completed chunk, the backend reads the current source chunk and verifies its SHA-256 before reusing the remote part. If any bytes changed, even with the same path, size, and mtime, the old state is deleted and the copy returns a retryable error so rclone restarts with a fresh multipart session; the uncertain old session is never finalized. A changed destination binding/revision also invalidates the state; an expired upload ID is refreshed and restarted safely from part 1. Successful completion removes the JSON state. Platforms without advisory file locking keep the ordinary multipart behavior but disable cross-process resume.
